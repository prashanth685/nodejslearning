          ...
          >
            ...
            <span
              style={{
                fontSize: 11,
                color:
                  i === current ? "#185FA5" : "var(--color-text-secondary)",
...
function FileDropZone({ file, onFile }) {
  ...
  return (
    ...
    >
      ...
      >
        or click to browse · .bin, .hex, .zip, .img supported
      </p>
      {file && (
        <div
          style={{
...
function FolderPicker({ token, onSelect, selectedFolder }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(
      `https://www.googleapis.com/drive/v3/files?q='${TARGET_FOLDER_ID}'+in+parents+and+mimeType='application/vnd.google-apps.folder'+and+trashed=false&fields=files(id,name,modifiedTime)&orderBy=name`,
      {
    ...
    <div>
        ...
        {["existing", "new"].map((m) => (
          <button
            ...
            style={{
              ...
              background:
                mode === m ? "#185FA5" : "var(--color-background-secondary)",
          ...
          >
            <i
              className={`ti ti-${m === "existing" ? "folder" : "folder-plus"}`}
              style={{ marginRight: 6 }}
              aria-hidden="true"
            />
      ...
      {mode === "new" ? (
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          <input
            type="text"
            placeholder="Company folder name (e.g. Acme Corp)"
            value={newName}
        ...
        >
          {folders.length === 0 && (
            ...
            >
              No company folders yet. Create one above.
            </p>
          )}
          {folders.map((f) => (
            <div
            ...
            >
              <i
                ...
                style={{ fontSize: 22, color: "#185FA5" }}
                aria-hidden="true"
              />
              <span
                style={{
...
export default function Hello() {
      ...
      tokenClient.current = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: (resp) => {
          if (resp.access_token) {
            ...
            setError("Sign-in failed or cancelled.");
            setSigning(false);
          }
        },
      });
  ...
  const signIn = () => {
    ...
    if (tokenClient.current) tokenClient.current.requestAccessToken();
      ...
      setError("Google SDK not ready yet. Try again.");
      setSigning(false);
    }
  };
    ...
    >
      ...
      <div style={{ marginBottom: "1.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <i
            className="ti ti-cpu"
            style={{ fontSize: 28, color: "#185FA5" }}
            aria-hidden="true"
          />
          <div>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 500 }}>
              FOTA Firmware Manager
            </h1>
            <p
              style={{
                margin: 0,
            ...
            >
              Upload firmware to Google Drive · Generate direct download links
            </p>
          </div>
          {!token ? (
            <button
            ...
            >
              <i
                ...
                style={{ fontSize: 16, color: "var(--color-text-success)" }}
                aria-hidden="true"
              />
              <span
                style={{
      ...
      {!token && (
        ...
        >
          <i
            ...
            aria-hidden="true"
          />
          Sign in with a Google account that has access to the FOTA Drive
          folder. No sign-in = no uploads.
        </div>
      )}
      ...
      {step === 1 && (
        <div>
          ...
          >
            Step 2 — Choose or create a company folder
          </p>
          <FolderPicker
            token={token}
            onSelect={setFolder}
          ...
          {folder && (
            ...
            >
              <i
                className="ti ti-folder-check"
                style={{ color: "var(--color-text-success)" }}
                aria-hidden="true"
              />
              <span>
                Selected: <strong>{folder.name}</strong>
              </span>
            </div>
          )}
          <div
      ...
      {step === 2 && (
        <div style={{ textAlign: "center", padding: "2rem 0" }}>
          <i
            ...
            aria-hidden="true"
          />
          <p style={{ fontWeight: 500, fontSize: 16, marginBottom: 4 }}>
            Uploading {file?.name}…
          </p>
          <p
            style={{
        ...
        <div>
          <div style={{ textAlign: "center", marginBottom: "2rem" }}>
            ...
            >
              <i
                className="ti ti-circle-check"
                style={{ fontSize: 32, color: "var(--color-text-success)" }}
                aria-hidden="true"
              />
            </div>
            <h2 style={{ margin: 0, fontWeight: 500, fontSize: 20 }}>
              Upload complete
            </h2>
            <p
              style={{
          ...
          >
            ...
            >
              Use this URL in your FOTA application to trigger an OTA update.
              The file is publicly accessible for download.
            </p>
          </div>
          ...
          >
            ...
            >
              <i className="ti ti-folder" aria-hidden="true" /> Open folder in
              Drive
            </a>
            <button
              onClick={reset}
